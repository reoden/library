...
template <typename Key, typename Data> struct HashMapBase {
  ...
  protected:
    ...
    template <typename K, enable_if<is_same<K, Key>::value, nullptr_t> = nullptr, enable_if<is_integral<K>::value, nullptr_t> = nullptr>
    inline u32 inner_hash(const K &key) const {
        return (randomized(key) * 11995408973635179863ULL) >> shift;
    }
    template <typename K, enable_if<is_same<K, Key>::value, nullptr_t> = nullptr, enable_if<is_integral<decltype(K::first)>::value, nullptr_t> = nullptr,
              enable_if<is_integral<decltype(K::second)>::value, nullptr_t> = nullptr>
    inline u32 inner_hash(const K &key) const {
    ...
    template <typename K, enable_if<is_same<K, Key>::value, nullptr_t> = nullptr,
              enable_if<is_integral<typename K::value_type>::value, nullptr_t> = nullptr>
    inline u32 inner_hash(const K &key) const {
    ...
    template <typename D = Data, enable_if<is_same<D, Key>::value, nullptr_t> = nullptr> inline u32 hash(const D &dat) const { return inner_hash(dat); }
    template <typename D = Data, enable_if<is_same<decltype(D::first), Key>::value, nullptr_t> = nullptr> inline u32 hash(const D &dat) const {
        return inner_hash(dat.first);
    }
  
    template <typename D = Data, enable_if<is_same<D, Key>::value, nullptr_t> = nullptr> inline Key dtok(const D &dat) const { return dat; }
    template <typename D = Data, enable_if<is_same<decltype(D::first), Key>::value, nullptr_t> = nullptr> inline Key dtok(const D &dat) const {
        return dat.first;
    }
  ...
  public:
    ...
    explicit HashMapBase() : cap(HASHMAP_DEFAULT_SIZE), s(0), data(cap), flag(cap), dflag(cap), shift(64 - std::__lg(cap)) {}
    ...
    void reserve(int n) {
        if(n <= 0) return;
        n = 1 << min(23, std::__lg(n) + 2);
        if(cap < u32(n)) reallocate(n);
    }
};
  
template <typename Key, typename Data>
uint64_t HashMapBase<Key, Data>::r = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count();
