 ...
 * Date: 2004-03-15
 * Description: $i1$, $i2$ are the indices to the closest pair of points in the
 * point vector $p$ after the call. The distance is returned. Time: O(n \log n)
 */
#pragma once
...
double closestpair(It begin, It end, It &i1, It &i2) {
  vector<It> xa, ya;
  assert(end - begin >= 2);
  for (It i = begin; i != end; ++i) xa.push_back(i), ya.push_back(i);
