 ...
 * have no colinear points. isct(a, b) returns a pair describing the
 * intersection of a line with the polygon: \begin{itemize*} \item $(-1, -1)$ if
 * no collision, \item $(i, -1)$ if touching the corner $i$, \item $(i, i)$ if
 * along side $(i, i+1)$, \item $(i, j)$ if crossing sides $(i, i+1)$ and $(j,
 * j+1)$. \end{itemize*} In the last case, if a corner $i$ is crossed, this is
 * treated as happening on side $(i, i+1)$. The points are returned in the same
 * order as the line hits the polygon. Status: fuzz-tested Time: O(N + Q \log n)
 */
#pragma once
...
struct HullIntersection {
  ...
  pii isct(P a, P b) {
    ...
    int x = bs2(f, j, a, b) % N, y = bs2(j, f, a, b) % N;
    if (a.cross(p[x], b) == 0 && a.cross(p[x + 1], b) == 0) return {x, x};
    if (a.cross(p[y], b) == 0 && a.cross(p[y + 1], b) == 0) return {y, y};
