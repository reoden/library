 ...
 * http://www.cs.berkeley.edu/~fateman/papers/fftvsothers.pdf For integers
 * rounding works if $(|a| + |b|)\max(a, b) < \mathtt{\sim} 10^9$, or in theory
 * maybe $10^6$.
 */
#include <bits/stdc++.h>
using namespace std;
...
struct num { /// start-hash
  int v;
  num(long long v_ = 0) : v(int(v_ % mod)) {
    if (v < 0)
      v += mod;
  }
...
vi multiply_mod(const vi &a, const vi &b, int m) {
  ...
  rep(i, 0, s) r[i] =
      int((ll(fa[i].x + 0.5) + (ll(fa[i].y + 0.5) % m << 15) +
           (ll(fb[i].x + 0.5) % m << 15) + (ll(fb[i].y + 0.5) % m << 30)) %
          m);
  return r;
} /// end-hash
#endif
