...
struct level_ancestor {
  ...
  vector<int> preorder;
  vector<int> idx;
  vector<pair<int, int>> heavyPar; // heavy parent, distance
  level_ancestor() : N(0) {}

  level_ancestor(const vector<int> &par)
      : N(int(par.size())), preorder(N), idx(N), heavyPar(N) {
    ...
    for (int i = 0; i < N; i++) {
      if (par[i] == -1) {
        y_combinator([&](auto self, int cur) -> void {
          sz[cur] = 1;
          for (int nxt : ch[cur]) {
            self(nxt);
            sz[cur] += sz[nxt];
          ...
          if (!ch[cur].empty()) {
            auto mit = max_element(ch[cur].begin(), ch[cur].end(),
                                   [&](int a, int b) { return sz[a] < sz[b]; });
            swap(*ch[cur].begin(), *mit);
          }
        })(i);
        y_combinator([&](auto self, int cur, int isRoot = true) -> void {
  ...
  int lca(int a, int b) const {
    ...
    while (true) {
      if (a > b) swap(a, b);
      assert(a <= b);
      if (a > b - heavyPar[b].second) { return preorder[a]; }
      b = heavyPar[b].first;
      if (b == -1) return -1;
    }
  }
