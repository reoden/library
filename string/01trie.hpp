...
struct BinaryTrie{
  ...
  int B;
  std::vector<BTNode> trie;
  BinaryTrie(){}
  BinaryTrie(int l) { B = l; trie.push_back(BTNode()); }
  void clear() { trie.clear(); trie.push_back(BTNode()); }
  ...
  int lower_bound(int x){ 
    ...
    for(int i = B - 1; i >= 0; i--){
      int e = (x >> i) & 1;
      if(e == 1 && trie[p].next[0] != -1) 
                ret += trie[trie[p].next[0]].size;
