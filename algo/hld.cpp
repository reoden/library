...
public:
  ...
  HeavyLightDecomposition(G &_g, int root = 0)
      : g(_g), id(0), size(g.size(), 0), depth(g.size(), 0), down(g.size(), -1),
        up(g.size(), -1), nxt(g.size(), root), par(g.size(), root) {
    dfs_sz(root);
    dfs_hld(root);
  }
  ...
  template <typename F> void path_query(int u, int v, bool vertex, const F &f) {
    int l = lca(u, v);
