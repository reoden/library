...
public:
   ...
   * \pmod b$ in the range $[0, 2b)$. Status: proven correct, stress-tested
   * Measured as having 4 times lower latency, and 8 times higher throughput,
   * see stress-test. Details: More precisely, it can be proven that the result
   * equals 0 only if $a = 0$, and otherwise lies in $[1, (1 + a/2^64) * b)$.
   */
...
public:
  ...
  explicit operator int() const { return v; }
  friend std::ostream &operator<<(std::ostream &out, const dynamic_modnum &n) {
    return out << int(n);
  }
...
template <typename T> struct mod_constraint {
  ...
  friend mod_constraint operator&(mod_constraint a, mod_constraint b) {
    ...
    return mod_constraint{a.v + extra * a.mod, a.mod * (b.mod / egcd.gcd)};
  }
};