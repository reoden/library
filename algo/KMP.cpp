...
namespace neal {
namespace KMP {
// Returns the next length to search from after starting from `len` and adding
// char `c`. Runs in worst case O(len) but amortizes to O(1) in most situations.
template <typename T, typename T_elem>
int get_link(const T &pattern, const vector<int> &fail, int len,
             const T_elem &c) {
...
// Computes the failure function on `pattern` so that we can search for it in
// future strings.
template <typename T> vector<int> compute_failure_function(const T &pattern) {
  // fail[i] = for the prefix [0, i) of `pattern`, the length of the longest
