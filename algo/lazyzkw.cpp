...
struct LazyZkwTree {
  ...
  void _check() {
    ...
    // m_com(m_defaultIncrement, m_defaultIncrement) == m_defaultIncrement);
    // if constexpr (std::is_invocable_v<_Mapping, _Fp, _Tp, int>)
    // assert(m_map(m_defaultIncrement, m_defaultValue, 1) == m_defaultValue);
  ...
  LazyZkwTree(uint32_t __length = 0, _Operation __op = _Operation(),
  ...
  void resize(uint32_t __length) {
    ...
    std::fill(m_sub.begin() + (1 << m_depth), m_sub.end(),
              _Tp_FpNode{m_defaultValue, m_defaultIncrement});
  ...
  void reset(_Iterator __first, _Iterator __last) {
    ...
    for (uint32_t i = 0; i < m_length; i++)
      m_sub[(1 << m_depth) + i] = {_Tp(__first[i]), m_defaultIncrement};
    std::fill(m_sub.begin() + (1 << m_depth) + m_length, m_sub.end(),
              _Tp_FpNode{m_defaultValue, m_defaultIncrement});
    for (uint32_t i = 1 << m_depth; --i;) {
      _update(i);
      m_sub[i].inc = m_defaultIncrement;
    ...
    if (__right > 1 << m_depth) do
        if (_Tp a(m_op(
          ...
          while (__right <= 1 << m_depth) {
            _pushDown(__right - 1);
            if (_Tp a(m_op(val, m_sub[(__right *= 2) - 1].val)); __judge(a))
              val = a, __right--;
          }
          return __right - (1 << m_depth);
        }
      while (__builtin_popcount(__right) > 1);
    return 0;
  }
...
LazyZkwTree(_Iterator, _Iterator, _Operation = _Operation(),
  ...
  /*
    ...
    auto Map = [](opNode x, i64 y, int size) { return y + x.a * size; };
    auto Com = [](opNode x, opNode y) {
      i64 a = x.a + y.a;
      return opNode{a};
    };
  */