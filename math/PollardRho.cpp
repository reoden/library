...
namespace OY {
template <typename _ModType> struct Barrett {
  _ModType m_P;
  __uint128_t m_Pinv;
  ...
  template <typename _Tp> constexpr _Tp divide(_Tp __a) const {
    if (__a < m_P) return 0;
  ...
  template <typename _Tp> constexpr std::pair<_Tp, _Tp> divmod(_Tp __a) const {
...
namespace OY {
template <typename _ModType> struct _MontgomeryTag;
template <> struct _MontgomeryTag<uint32_t> {
...
template <> struct _MontgomeryTag<uint64_t> {
...
template <typename _ModType> struct Montgomery {
  ...
  constexpr explicit Montgomery(_ModType __P)
      : m_P(__P), m_Pinv(__P), m_Ninv(-_LongType(__P) % __P), m_brt(__P) {
    for (int i = 0; i < _MontgomeryTag<_ModType>::inv_loop; i++)
  ...
  constexpr _FastType raw_init(uint64_t __a) const {
    return reduce(_LongType(__a) * m_Ninv);
  }

  constexpr _FastType reduce(_LongType __a) const {
    _FastType res = (__a >> _MontgomeryTag<_ModType>::length) -
                    _ModType(_LongType(_ModType(__a) * m_Pinv) * m_P >>
...
namespace OY {
template <typename _Elem> constexpr bool isPrime(_Elem n) {
  if (std::is_same_v<_Elem, uint32_t> || n <= UINT32_MAX) {
    if (n <= 1) return false;
    if (n == 2 || n == 7 || n == 61) return true;
    if (n % 2 == 0) return false;
    Barrett32 brt(n);
    uint32_t d = (n - 1) >> __builtin_ctzll(n - 1);
    for (auto &&a : {2, 7, 61}) {
...
struct Pollard_Rho {
  ...
  template <typename _Elem> static _Elem pick(_Elem __n) {
    ...
    _Elem v0, v1 = mg.init(distribute(s_rander)), prod = mg.init(1),
              c = mg.init(distribute(s_rander));
  ...
  template <typename _Elem> static auto decomposite(_Elem __n) {
    struct node {
      _Elem prime;
      uint32_t count;
    };
    ...
    if (__n % 2 == 0) {
      res.push_back({2, uint32_t(__builtin_ctzll(__n))});
      __n >>= __builtin_ctzll(__n);
    }
  ...
  template <typename _Elem> static std::vector<_Elem> getFactors(_Elem __n) {
  ...
  template <typename _Elem> static _Elem EulerPhi(_Elem __n) {
